...
function create(opts) {
  ...
  return {
    ...
    recordBackorder: async function (input) {
      ...
      // Atomic cap claim. The early check above reads a snapshot; two
      // concurrent calls can both pass it and then both increment, pushing
      // pending past the cap. Fold the cap into the WHERE so only an
      // increment that KEEPS pending within the cap applies, and gate on
      // rowCount: a losing claim means a concurrent line already took the
      // remaining capacity (or the sku was deactivated). The line row was
      // inserted above, so on a loss remove it — it must never count as a
      // committed backorder without a matching counter increment — then
      // refuse with the current reason.
      ...
      if (Number((claim && claim.rowCount) || 0) !== 1) {
        await query("DELETE FROM backorder_lines WHERE id = ?1", [id]);
