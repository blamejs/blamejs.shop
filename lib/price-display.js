...
async function _resolveRateBps(country, taxRates) {
  ...
  if (typeof taxRates === "object") {
    ...
    var rate = taxRates[country];
    if (rate == null) return null;
    return _bps(rate, "taxRates." + country);
  }
  throw new TypeError(
...
function create(opts) {
  ...
  if (!query) {
    query = function (sql, params) {
      return b.externalDb.query(sql, params);
    };
  }
    ...
    if (tax && mode) {
      ...
      if (rateBps != null) {
        if (mode === "tax_included") {
          ...
          var netMoney = b.money.fromMinorUnits(BigInt(bo.net_minor), input.currency);
