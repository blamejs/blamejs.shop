...
function mount(router, deps) {
  ...
  if (discounts) {
    router.post("/admin/discounts", W("discount.create", async function (req, res) {
      var d = await discounts.create(req.body || {});
      _json(res, 201, d);
      return d;
    }));

    router.get("/admin/discounts", R(async function (req, res) {
      var url = req.url ? new URL(req.url, "http://localhost") : null;
      var limitS  = url && url.searchParams.get("limit");
      ...
      var listOpts = { limit: limit, offset: offset };
      if (activeS === "1" || activeS === "true")  listOpts.active = true;
      if (activeS === "0" || activeS === "false") listOpts.active = false;
      var page = await discounts.list(listOpts);
      _json(res, 200, page);
    }));

    router.get("/admin/discounts/:id", R(async function (req, res) {
      var d = await discounts.get(req.params.id);
      if (!d) return _problem(res, 404, "discount-not-found");
      _json(res, 200, d);
    }));

    router.patch("/admin/discounts/:id", W("discount.update", async function (req, res) {
      var d = await discounts.update(req.params.id, req.body || {});
      if (!d) return _problem(res, 404, "discount-not-found");
      _json(res, 200, d);
      return d;
    }));

    router.delete("/admin/discounts/:id", W("discount.delete", async function (req, res) {
      var ok = await discounts.delete(req.params.id);
      if (!ok) return _problem(res, 404, "discount-not-found");
      _json(res, 200, { ok: true });
      return { id: req.params.id };
    }));

    router.get("/admin/discounts/:id/redemptions", R(async function (req, res) {
      var url = req.url ? new URL(req.url, "http://localhost") : null;
      var limitS = url && url.searchParams.get("limit");
      var limit  = limitS == null ? 50 : parseInt(limitS, 10);
      var page = await discounts.redemptions(req.params.id, { limit: limit });
      _json(res, 200, page);
    }));
  }
