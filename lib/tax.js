...
function validateVatId(vat_id, country_code) {
  ...
  // prefix is the ISO code for every registry EXCEPT two, where the
  // filed prefix differs from the ISO 3166-1 code: Greece files under
  // "EL" (ISO "GR"), and Switzerland under "CHE" (three chars, ISO
  // "CH"). Guessing the length (2 vs 3) or matching on the ISO code
  // corrupts those two — it drops the leading digit of a GR number and
  // leaves the stray "E" on a CH number.
  var prefix;
  if (country_code === "GR") prefix = "EL";
  else if (country_code === "CH") prefix = "CHE";
  else prefix = country_code;
  return {
    ok:         true,
    country:    country_code,
