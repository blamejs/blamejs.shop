    ...
    tickRetries: async function (input) {
      ...
      for (var i = 0; i < due.rows.length; i += 1) {
        ...
        try {
          ...
          // The claim above nulled next_retry_at to take this row out of
          // the due set. If the advance then throws (a malformed policy
          // parse, a DB write failure mid-attempt) BEFORE _tickAdvance
          // re-stamps next_retry_at, the enrollment would be stranded
          // active with next_retry_at = NULL — and the due SELECT requires
          // next_retry_at IS NOT NULL, so no later tick would ever pick it
          // up and its recovery retries would silently stop. Restore the
          // observed next_retry_at (only while still NULL, so a partial
          // re-stamp isn't clobbered) so a later tick re-attempts.
          await query(
