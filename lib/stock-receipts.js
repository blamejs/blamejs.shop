...
function create(opts) {
  ...
  async function recordReceiptScan(input) {
    ...
    if (!b.crypto.timingSafeEqual(receipt.token_hash, hash)) {
    ...
    var scanId = b.uuid.v7();
    var uaHash = ua != null ? b.crypto.namespaceHash(UA_NAMESPACE, ua) : null;
    var ipHash = ip != null ? b.crypto.namespaceHash(IP_NAMESPACE, ip) : null;
  ...
  async function getReceiptByToken(token) {
    ...
    if (!row) return null;
    if (!b.crypto.timingSafeEqual(row.token_hash, hash)) return null;
