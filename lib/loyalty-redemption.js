...
function create(opts) {
  ...
  async function cancelRedemption(input) {
    ...
    //
    // The status flip above is the cancellation fence, but creditBalance is a
    // SEPARATE non-idempotent statement (D1 has no interactive transactions).
    // If it throws, the row is already 'cancelled' while the points were never
    // returned, and a retry short-circuits at the status!=='active' guard —
    // stranding the points. Crediting BEFORE the flip is the wrong fix (a
    // retry would double-credit, since creditBalance has no fence). So on a
    // credit failure we REVERT the flip back to 'active', mirroring how
    // redeemForCustomer reverses its burn on a later-step failure: the points
    // stay put, the operator's error surfaces, and the cancel can be retried.
    try {
      ...
      try {
        await query(
          "UPDATE loyalty_redemptions SET status = 'active', cancel_reason = NULL " +
