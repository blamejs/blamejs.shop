...
function create(opts) {
  ...
  return {
    ...
    // ---- clearFailures --------------------------------------------------
    //
    // Delete the recent failed-attempt rows for an account and/or IP after a
    // SUCCESSFUL sign-in, so a legitimate operator who mistyped a few times
    // isn't held under the rolling lockout window on the next attempt. Called
    // on the success path only; a no-op when neither key is given. Returns the
    // row count cleared.
    clearFailures: async function (input) {
      ...
      var oid = input.operator_id == null ? null : _uuid(input.operator_id, "operator_id");
      var ip  = input.ip_hash == null ? null
        : _requiredString(input.ip_hash, "ip_hash", MIN_IP_HASH_LEN, MAX_IP_HASH_LEN);
      if (oid == null && ip == null) return { cleared: 0 };
      var conds = [];
      var params = [];
      if (oid != null) { params.push(oid); conds.push("operator_id = ?" + params.length); }
      if (ip  != null) { params.push(ip);  conds.push("ip_hash = ?" + params.length); }
      var res = await query(
        "DELETE FROM operator_failed_logins WHERE " + conds.join(" OR "),
        params,
      );
