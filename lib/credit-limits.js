...
function create(opts) {
  ...
  // charges. Payments are revolving (order_id=NULL) and apply global-FIFO,
  // oldest order first; a release nets only against its OWN order. So an
  // order's unpaid = its net charge (charges+holds − its own releases)
  // minus the FIFO slice of the payment pool that lands on it. The FIFO
  // slice is the overlap of the order's position interval
  // [cum_start, cum_start+net) — cum_start = the cumulative net of all
  // strictly-earlier orders, by (first-charge time, order_id) — with the
  // paid interval [0, totalPayments). Expressed as one correlated subquery
  // (CTE + window function) so it composes inside the SAME single-statement
  // atomic write as _BAL_SUB: a concurrent payment/release commits first
  // and this subquery re-evaluates against it, so race-safety is preserved
  // with no app-side read-then-write. References ?2 (customer_id) + ?4
  // (order_id), both bound by _appendTxnAtomic. The agingReport recompute
  // uses the IDENTICAL attribution model so the two surfaces never disagree.
  var _UNPAID_SUB =
    "COALESCE((WITH " +
    "totals AS (SELECT COALESCE(SUM(amount_minor), 0) AS pay " +
    "  FROM credit_transactions WHERE customer_id = ?2 AND kind = 'payment'), " +
    "order_slices AS (SELECT order_id, " +
    "  MIN(CASE WHEN kind IN ('charge', 'hold') THEN occurred_at END) AS at, " +
    "  COALESCE(SUM(CASE WHEN kind IN ('charge', 'hold') THEN amount_minor " +
    "                    WHEN kind = 'release' THEN -amount_minor END), 0) AS net " +
    "  FROM credit_transactions WHERE customer_id = ?2 AND kind IN ('charge', 'hold', 'release') " +
    "  GROUP BY order_id HAVING net > 0), " +
    "ordered AS (SELECT order_id, net, " +
    "  COALESCE(SUM(net) OVER (ORDER BY at, order_id ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), 0) AS cum_start " +
    "  FROM order_slices) " +
    "SELECT o.net - MAX(0, MIN(o.cum_start + o.net, (SELECT pay FROM totals)) - MAX(o.cum_start, 0)) " +
    "FROM ordered o WHERE o.order_id = ?4), 0)";
