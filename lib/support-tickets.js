...
function create(opts) {
  ...
  async function _writeStatusHistory(ticketId, fromStatus, toStatus, reason, ts) {
    await query(
      ...
      "VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
      [_b().uuid.v7(), ticketId, fromStatus, toStatus, reason, ts],
    );
  }
  ...
  return {
    ...
    open: async function (input) {
      ...
      await query(
        ...
        [_b().uuid.v7(), id, customerId, body, ts],
      );
    ...
    reply: async function (input) {
      ...
      await query(
        ...
        [_b().uuid.v7(), ticketId, author, authorId, body, internal, ts],
      );
