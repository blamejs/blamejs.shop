...
function _compileRegex(source) {
  ...
  // Extend the backreference / lookaround screen above with the remaining
  // catastrophic-backtracking classes. This regex runs against the request
  // path in `resolveForPath`, so a super-linear pattern is a DoS on every
  // matching request.
  //
  // b.guardRegex.assertSafe is the screen. It used to reject linear patterns
  // like `/blog(?:/page/\d+)?` — reading the `?` of a `(?:` group prefix, and
  // a bounded trailing `?`, as quantifiers — so this module carried its own
  // nested-quantifier walker instead. That was fixed upstream, and the
  // primitive additionally refuses a class the local walker missed: an
  // ambiguous alternation under an unbounded quantifier (`(a|a)*`), which
  // backtracks exponentially just as `(a+)+` does.
  try {
    b.guardRegex.assertSafe(source);
  } catch (e) {
    throw new TypeError(
      "siteRedirects: regex source rejected — catastrophic-backtracking (ReDoS) " +
      "surface; rewrite as `prefix` / `exact`, or split into multiple rows — " +
      (e && e.message ? e.message : "unsafe pattern")
    );
  }
  return compiled;
}
