...
function create(opts) {
  ...
  return {
    ...
    compareOrders: async function (orderIds) {
      ...
      var idA = _uuid(orderIds[0], "orderIds[0]");
      var idB = _uuid(orderIds[1], "orderIds[1]");
      var batches = await Promise.all([
        _collectAll(idA),
        _collectAll(idB),
      ]);
      ...
      return {
        a: { order_id: idA, timeline: _stripInternalFields(batches[0]) },
        b: { order_id: idB, timeline: _stripInternalFields(batches[1]) },
      };
    },
