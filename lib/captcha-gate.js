...
function _publicKey(s) {
  ...
  textGuard.freeText(s, "captchaGate: public_key");
  return s;
}
...
function _secretKeyRaw(s) {
  ...
  textGuard.freeText(trimmed, "captchaGate: secret_key");
  return trimmed;
}
...
function _token(s) {
  ...
  textGuard.freeText(s, "captchaGate: token");
  return s;
}
...
function _optionalHash(s, label) {
  ...
  if (textGuard.hasCodepointThreat(s)) {
    throw new TypeError("captchaGate: " + label + " contains control bytes");
  }
  return s;
}
...
function _optionalSessionId(s) {
  ...
  textGuard.freeText(s, "captchaGate: session_id");
  return s;
}
