...
async function _importRows(query, customers, state, maxRows, maxErrors, input, source) {
  ...
  var inputByteCount;
  try {
    inputByteCount = Buffer.byteLength(b.safeJson.stringify(input.rows, { canonical: true }), "utf8");
  } catch (_e) {
    inputByteCount = 0;
  }
...
async function _importFromCsv(query, customers, state, maxRows, maxErrors, stream) {
  ...
  var guardRv = b.guardCsv.validate(input, { profile: "strict" });
  if (!guardRv.ok) {
  ...
  var records;
  try {
    records = b.csv.parse(input, { header: true, maxRows: maxRows + 1 });
  } catch (e) {
