...
function _b() {
  ...
  // initializing the vendored blamejs tree (the tests pass their
  // own `query` function and never touch the runtime singleton).
...
function _contentType(s) {
  try {
    return _b().guardMime.sanitize(s, { profile: "strict" });
  } catch (e) {
...
function _genId() { return _b().uuid.v7(); }
...
function create(opts) {
  ...
  var inventory = _inventoryModule(query);
  ...
  return {
    ...
    list: async function (opts) {
      ...
      if (opts.cursor != null) {
        ...
        try {
          var state = _b().pagination.decodeCursor(opts.cursor, cursorSecret);
    ...
    update: async function (id, patch) {
      ...
      function _addSet(col, val) {
        ...
        _b().safeSql.assertOneOf(col, ALLOWED_PRODUCT_COLUMNS);
...
function _inventoryModule(query) {
  return {
