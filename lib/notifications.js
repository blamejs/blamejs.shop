...
function create(opts) {
  ...
  return {
    ...
    markSent: async function (id, options) {
      ...
      // Atomic claim: fold the observed-status predicate into the UPDATE so
      // the transition is a single serialized claim, not a read-then-write.
      // Two concurrent dispatch callbacks (or a markSent racing a markFailed)
      // can't both write the row last-write-wins; rowCount is the claim signal.
      var upd = await query(
