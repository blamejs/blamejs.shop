...
function create(opts) {
  ...
  async function _pushPlanToStripe(stripeSubscriptionId, newStripePriceId, prorationBehavior, planChangeId) {
    ...
    try {
      ...
      rErr.cause = e;
      // The repricing was never sent — the read that precedes it is what
      // failed — so a caller holding a claim can undo it without diverging.
      rErr.indeterminate = false;
      throw rErr;
    }
    ...
    if (!items.length || !items[0] || !items[0].id) {
      ...
      // Refused here, before anything crossed the wire.
      noItem.indeterminate = false;
      throw noItem;
    }
    ...
    try {
      ...
      // Name which of the two this was. "Rejected" asserts something about
      // Stripe's answer, and on a timeout there was no answer to report.
      var unknown = _pushOutcomeIndeterminate(e2);
      var uErr = new Error(
        (unknown
          ? "planChanges: no answer from Stripe on the plan swap — "
          : "planChanges: Stripe rejected the plan swap — ") + (e2 && e2.message || e2),
      );
      uErr.code = "PLAN_CHANGE_STRIPE_PUSH_FAILED";
      uErr.cause = e2;
      uErr.indeterminate = unknown;
      throw uErr;
    }
  }

  // Did the provider call leave the transition in a KNOWN state?
  //
  // A 4xx is Stripe's considered rejection: it received the request, refused
  // it, and billed nothing — so the caller can safely undo its local claim. A
  // transport error (timeout, connection reset) or a 5xx is indeterminate: the
  // repricing may well have landed and cut its proration invoice and we simply
  // never heard the answer. Undoing the claim on one of those would risk
  // showing the outgoing plan for a subscription Stripe has already repriced
  // and charged, so the claim is HELD instead and the scheduler sweep replays
  // it under the same idempotency key until Stripe gives a definite answer.
  function _pushOutcomeIndeterminate(err) {
    var status = err && err.statusCode;
    if (typeof status !== "number") return true;   // transport-level — no verdict was ever reached
    return !(status >= 400 && status < 500);       // 4xx decided it; 5xx did not
  }

  // Drop a held transition claim, fenced on the change id the caller believes
  // it holds, so a concurrent settle is never clobbered.
  async function _releaseClaim(subscriptionId, changeId, ts) {
    await query(
      "UPDATE subscriptions SET plan_transition_change_id = NULL, updated_at = ?1 " +
      "WHERE id = ?2 AND plan_transition_change_id = ?3",
      [ts, subscriptionId, changeId],
    );
  }
  ...
  return {
    ...
    executeChange: async function (input) {
      ...
      if (stripeBacked) {
        ...
        var stripeClaim = await query(
          "UPDATE subscriptions SET plan_id = ?1, plan_transition_change_id = ?2, updated_at = ?3 " +
          "WHERE id = ?4 AND plan_id = ?5 AND plan_transition_change_id IS NULL",
          [newPlanId, id, ts, subscriptionId, sub.plan_id],
        );
        if (!b.sql.casWon(stripeClaim).won) {
          // Did not get the transition — either a concurrent change already
          // moved the plan, or one is still settling at Stripe. Read the claim
          // column back so the voided row records WHICH, since the two mean
          // very different things to an operator reading the history. Neither
          // pushes to Stripe or charges anything here; void this row rather
          // than leave a phantom 'executed' change carrying charge/credit
          // columns for a transition that did not happen (mirrors the
          // non-Stripe path and applyScheduledChanges).
          var lostRead = await query(
            "SELECT plan_transition_change_id FROM subscriptions WHERE id = ?1",
            [subscriptionId],
          );
          var lostRow = (lostRead && lostRead.rows && lostRead.rows[0]) || null;
          await query(
            "UPDATE subscription_plan_changes SET status = 'cancelled', cancelled_at = ?1, " +
            "cancel_reason = ?2 WHERE id = ?3",
            [
              _now(),
              lostRow && lostRow.plan_transition_change_id ? "transition-in-flight" : "transition-lost",
              id,
            ],
          );
          return await _getById(id);
        }
        // Won the claim — reprice at Stripe keyed on THIS change row's id.
        // The row id is the transition's durable identity: a replay of this
        // same unsettled transition (the caller retrying, or the stranded
        // sweep below) presents the same key and Stripe dedupes it, while a
        // later, genuinely separate change between the same two plans in the
        // same billing period is a different row and so bills as it should.
        // Keying on the transition's SHAPE instead — subscription + from/to
        // price + period — collapses A→B, B→A, A→B within one period onto a
        // single key, and Stripe would replay the third as the first, moving
        // the plan locally while silently skipping its always_invoice
        // proration. _pushPlanToStripe prefixes "planchange:".
        try {
          await _pushPlanToStripe(sub.stripe_subscription_id, toPlanImmediate.stripe_price_id, "always_invoice", id);
        } catch (e) {
          if (e && e.indeterminate) {
            // No verdict from Stripe — a timeout or a 5xx. The repricing may
            // have landed and cut its invoice. Rolling back here could park
            // the shop on the outgoing plan for a subscription Stripe has
            // already charged, so HOLD the claim and leave the row executed;
            // reconcileStrandedTransitions replays it under this same key
            // until Stripe answers definitively.
            _log.error("planChanges: immediate Stripe plan swap outcome unknown — holding the claim", {
              plan_change_id:  id,
              subscription_id: subscriptionId,
          ...
          // Stripe decided: it refused the swap and billed nothing. Release
          // the claim and roll plan_id back (guarded on the just-applied plan
          // AND this change's claim so a concurrent change is never
          // clobbered), then void this row so the shop never shows a repricing
          // Stripe did not bill, and rethrow so the route surfaces a clean retry.
          await query(
            "UPDATE subscriptions SET plan_id = ?1, plan_transition_change_id = NULL, updated_at = ?2 " +
            "WHERE id = ?3 AND plan_id = ?4 AND plan_transition_change_id = ?5",
            [sub.plan_id, _now(), subscriptionId, newPlanId, id],
          );
          await query(
            "UPDATE subscription_plan_changes SET status = 'cancelled', cancelled_at = ?1, " +
            "cancel_reason = ?2 WHERE id = ?3",
        ...
        // Stripe confirmed the repricing — release the claim so the next
        // transition for this subscription can proceed.
        await _releaseClaim(subscriptionId, id, _now());
        return await _getById(id);
      }
    ...
    // Scheduler-callable. Settles subscriptions stranded mid-transition.
    //
    // An immediate Stripe-backed change HOLDS its claim when the provider
    // call's outcome is unknown (a timeout, a 5xx), because rolling back could
    // park the shop on the outgoing plan for a subscription Stripe has already
    // repriced and charged. A held claim also blocks every later change for
    // that subscription, so something has to resolve it. This replays the
    // repricing under the SAME idempotency key the original used: if that call
    // did land, Stripe returns the recorded response and nothing is billed
    // twice; if it never landed, this is the call that lands it. Either way
    // the subscription converges on the plan its change row names and the
    // claim is released.
    //
    // Returns the change rows it settled. One unresolved subscription never
    // aborts the run — it keeps its claim and the next sweep tries again.
    reconcileStrandedTransitions: async function (input) {
      if (!input || typeof input !== "object") {
        throw new TypeError("planChanges.reconcileStrandedTransitions: input object required");
      }
      var now = _epochMs(input.now, "now");
      var stranded = await query(
        "SELECT * FROM subscriptions WHERE plan_transition_change_id IS NOT NULL ORDER BY id ASC",
        [],
      );
      var settled = [];
      for (var i = 0; i < stranded.rows.length; i += 1) {
        var s   = stranded.rows[i];
        var cid = s.plan_transition_change_id;
        var chg = await _getById(cid);

        // A claim pointing at a change row that is gone, already resolved, or
        // was never a Stripe repricing can never be settled by a replay.
        // Release it rather than block the subscription forever.
        if (!chg || chg.status !== "executed" || !_isStripeBacked(s)) {
          await _releaseClaim(s.id, cid, now);
          continue;
        }
        var toPlan = await _getPlan(chg.to_plan_id);
        if (!toPlan || !toPlan.stripe_price_id) {
          // Unreplayable: the target plan carries no Stripe price. Free the
          // subscription and leave the row executed for an operator to inspect.
          _log.error("planChanges: stranded transition targets a plan with no stripe_price_id", {
            plan_change_id:  cid,
        ...
        try {
          await _pushPlanToStripe(s.stripe_subscription_id, toPlan.stripe_price_id, "always_invoice", cid);
        } catch (e) {
          if (e && e.indeterminate) {
            // Still no verdict — keep holding and try again next sweep.
            _log.error("planChanges: stranded transition still unresolved at Stripe", {
              plan_change_id:  cid,
              subscription_id: s.id,
              err:             (e && e.message) || String(e),
            });
          ...
          // Stripe refused it outright, so it billed nothing: undo the
          // transition and void the row, both fenced on this claim.
          _log.error("planChanges: stranded transition refused by Stripe — reverting", {
            plan_change_id:  cid,
            subscription_id: s.id,
            err:             (e && e.message) || String(e),
          });
          await query(
            "UPDATE subscriptions SET plan_id = ?1, plan_transition_change_id = NULL, updated_at = ?2 " +
            "WHERE id = ?3 AND plan_id = ?4 AND plan_transition_change_id = ?5",
            [chg.from_plan_id, now, s.id, chg.to_plan_id, cid],
          );
          await query(
            "UPDATE subscription_plan_changes SET status = 'cancelled', cancelled_at = ?1, " +
            "cancel_reason = ?2 WHERE id = ?3",
            [now, "stripe-push-failed", cid],
          );
          continue;
        }
        await _releaseClaim(s.id, cid, now);
        settled.push(await _getById(cid));
      }
      return settled;
    },
