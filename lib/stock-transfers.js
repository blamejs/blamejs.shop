...
function create(opts) {
  ...
  return {
    ...
    reconcile: async function (input) {
      ...
      var discrepancies = [];
      // Credit the destination one line at a time. The claim above already
      // flipped the status, so a concurrent call can never reach this loop for
      // the same transfer. A failure mid-loop leaves the operator with a
      // partially-credited destination + a 'reconciled' status; every
      // adjustStock that landed shows up in the audit log, and the FSM gate
      // refuses any retry (no second credit is possible).
