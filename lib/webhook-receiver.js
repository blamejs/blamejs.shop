 ...
 *     - `b.crypto.hmac`            — HMAC-SHA-256 over the signed
 *                                    `<timestamp>.<raw-body>` payload.
 *     - `b.uuid.v7`                — row ids.
 *
 *   Surface:
...
// Recompute the expected `v1` tag. In this scheme the signed payload is
// `<timestamp>.<raw-body>`, keyed on the endpoint secret.
//
// Both arguments go in exactly as received. The body is signed as BYTES: a
// body carrying anything that is not valid UTF-8 loses those bytes to U+FFFD
// on a string round-trip, which changes what gets hashed and rejects a
// delivery the sender signed correctly. The timestamp is signed as the raw
// header text rather than a reparsed integer, so the digest is taken over
// what the sender actually put on the wire.
//
// b.webhookHmac.verify implements this entire scheme — header parse, replay
// window, key-rotation multi-signature, constant-time compare — and is the
// primitive this should compose. It reads the clock through Date.now() with
// no injection point, while this receiver runs on an operator-supplied clock
// (opts.now) and a per-source replay window, so its window check would fire
// against real time regardless of the configured tolerance. Requested
// upstream as blamejs/blamejs#557; drop this helper and call verify() once it
// accepts a clock.
function _expectedSignatureHex(secretPlaintext, timestampRaw, bodyBytes) {
  var signed = Buffer.concat([
    Buffer.from(timestampRaw + ".", "utf8"),
    bodyBytes,
  ]);
  return b.crypto.hmac(secretPlaintext, signed, "sha256");
}
...
function create(opts) {
  ...
  return {
    ...
    verifyAndPersist: async function (input) {
      ...
      // `<timestamp>.<body>` payload with HMAC-SHA-256 under the
      // secret. We recompute over the received bytes and constant-time-
      ...
      var expected = _expectedSignatureHex(matched.plaintext, tsStr, bodyBytes);
      var sigOk = b.crypto.timingSafeEqual(supplied, expected);
      if (!sigOk) {
        return {
          ok:                       false,
          event_id:                 null,
