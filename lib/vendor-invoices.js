...
function create(opts) {
  ...
  async function recordInvoice(input) {
    ...
    var priorCur = await query(
      "SELECT currency FROM vendor_invoices WHERE vendor_slug = ?1 LIMIT 1",
      [vendorSlug],
    );
    if (priorCur.rows.length && priorCur.rows[0].currency !== currency) {
      var curErr = new Error(
        "vendor-invoices.recordInvoice: vendor " + JSON.stringify(vendorSlug) +
        " already bills in " + priorCur.rows[0].currency + "; this invoice is " +
