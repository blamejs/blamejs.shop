  ...
  return {
    ...
    subscribe: async function (input) {
      ...
      if (existing) {
        ...
        if (stillLive) {
          // A signed-in resubmit of an alert first created anonymously
          // ADOPTS the live row: backfilling the customer link is what
          // makes the subscription reachable from a customer-keyed
          // privacy request (export + erasure) — without it the row's
          // plaintext address would outlive an account erasure on this
          // common anonymous-then-signed-in path. Write-once: a row that
          // already carries a customer link is never re-keyed by a later
          // submission.
          if (customerId != null) {
            await query(
              "UPDATE stock_alerts SET customer_id = ?1 WHERE id = ?2 AND customer_id IS NULL",
