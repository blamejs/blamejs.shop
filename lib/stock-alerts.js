...
// shop primitive; defers the `require("./index")` past module-eval
// so the entry-point's lazy assembly of the framework + the shop
// surface doesn't trip a load-order cycle.
var bShop;
function _b() {
...
function _hashEmail(normalised) {
  return _b().crypto.namespaceHash(EMAIL_NAMESPACE, normalised);
}

function _hashToken(plaintext) {
  return _b().crypto.namespaceHash(TOKEN_NAMESPACE, plaintext);
}
