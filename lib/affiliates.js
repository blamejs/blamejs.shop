...
function create(opts) {
  ...
  // The affiliate's current chain tip — the latest commission's row_hash (which
  // the next commission links off as its prev_hash) plus its occurred_at (so the
  // writer can keep the per-affiliate occurred_at strictly monotonic). No rows
  // (or a legacy pre-chain row whose row_hash is NULL) anchors the chain fresh
  // from ZERO_HASH; verifyChain reports the unhashed legacy prefix as
  // unverifiable rather than trusting it.
  async function _readCommissionTip(affiliateId) {
    var r = await query(
      "SELECT row_hash, occurred_at FROM affiliate_commissions WHERE affiliate_id = ?1 " +
      "ORDER BY occurred_at DESC, id DESC LIMIT 1",
      [affiliateId],
    );
    if (!r.rows.length) return { row_hash: ZERO_HASH, occurred_at: null };
    return {
  ...
  // Keep each affiliate's commission occurred_at STRICTLY monotonic, so the
  // chain's (occurred_at, id) read order always matches its prev_hash linkage —
  // two commissions in the same millisecond (or an out-of-order operator-
  // supplied occurred_at) would otherwise sort differently from the chain and
  // make the tip read ambiguous. A genuine forward write lands at its requested
  // time; only a tie or a backdate is bumped to prior + 1.
  function _resolveCommissionOccurredAt(requestedTs, latestTs) {
  ...
  return {
    ...
    recordCommissionEvent: async function (input) {
      ...
      for (var attempt = 0; attempt < COMMISSION_CHAIN_ATTEMPTS; attempt += 1) {
        ...
        var rowOccurredAt = _resolveCommissionOccurredAt(occurredAt, tip.occurred_at);
        var id = b.uuid.v7();
        ...
        try {
          await query(
            ...
            " prev_hash, row_hash) " +
            "VALUES (?1, ?2, ?3, ?4, ?5, ?6, 'pending', ?7, NULL, NULL, NULL, NULL, ?8, ?9)",
            [id, orderId, affiliateId, orderTotal, commissionMinor, currency, rowOccurredAt, prevHash, rowHash],
          );
