...
function create(opts) {
  ...
  return {
    ...
    updateAffiliate: async function (affiliateId, patch) {
      ...
      // Re-validate the EFFECTIVE commission_value against the effective
      // kind whenever the kind OR the value is patched — not only when the
      // value is in the patch. A kind-only switch (e.g. an
      // amount_per_order_minor value, legal up to the 1e11 flat cap,
      // switched to percent_bps where the cap is 10000 = 100%) must be
      // rejected by _commissionValue rather than silently persisting a value
      // that is now thousands of times over the bps cap and would mint a
      // runaway commission. Writing the re-validated current value on a
      // kind-only patch is a no-op when it is still legal.
      if (patch.commission_kind != null || patch.commission_value != null) {
        var effectiveValue = patch.commission_value != null
          ? patch.commission_value
          : Number(current.commission_value);
        _set("commission_value", _commissionValue(effectiveValue, effectiveKind));
      }
