...
// Fetch (or reuse) the PayPal client-credentials bearer token.
//
// The exchange runs behind b.safeAsync.keyedSerializer so concurrent callers
// on a cold or just-expired cache do not each mint a token: the first through
// performs the exchange, the rest queue behind it and find the cache warm on
// the re-check inside the gate. Without that, a burst of checkouts issued N
// tokens and PayPal rate-limits the token endpoint well below the checkout
// rate. The fast path — a token already valid — is read before the gate, so a
// warm cache costs nothing.
//
// b.auth.oauth's clientCredentialsManager covers this whole shape, including
// the 429 backoff this does not have. It is not composed here because it dials
// through its own http-client instance: adopting it would drop the SSRF
// allowedHosts pin on the token endpoint, this adapter's circuit breaker and
// bounded retry, and the injected opts.httpClient the tests drive. Asked
// upstream for an injectable client in blamejs/blamejs#561.
async function _paypalToken(opts, state) {
  if (state.token && state.now() < state.tokenExpiresAt) return state.token;
  return state.tokenGate.run("paypal-token", function () {
    return _paypalTokenExchange(opts, state);
  });
}

async function _paypalTokenExchange(opts, state) {
  var now = state.now();
  // Re-check inside the gate: a caller that queued behind an in-flight
  // exchange finds the token it was waiting for already cached.
...
function paypal(opts) {
  ...
  var state = {
    ...
    token:          null,
    tokenExpiresAt: 0,
    // Serializes the token exchange so N concurrent PayPal calls on a cold or
    // just-expired cache mint ONE token instead of N. See _paypalToken.
    tokenGate:      b.safeAsync.keyedSerializer(),
  };
