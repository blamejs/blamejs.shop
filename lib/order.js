...
function create(opts) {
  ...
  return {
    ...
    transition: async function (orderId, event, opts2) {
      ...
      if (result.to === "cancelled" || result.to === "refunded") {
        ...
        if (refreshed && refreshed.customer_id && _refTotal > 0) {
          ...
          // The claw ratios against the SUBTOTAL (the goods base the points
          // were earned on, per the awardForEvent call above), not the grand
          // total: a full-refund death edge refunds the whole order, so
          // effRefunded (capped at the subtotal) reaches the subtotal and the
          // claw is 100%. Detached + drop-silent; clawed_points makes the claw
          // idempotent and convergent across partial slices.
          var _revSubtotal = Number(refreshed && refreshed.subtotal_minor) || 0;
          if (_revSubtotal > 0 && loyaltyEarnRules && typeof loyaltyEarnRules.reverseForEventProRata === "function") {
            Promise.resolve().then(function () {
              return loyaltyEarnRules.reverseForEventProRata({
                customer_id:       _revCustomer,
                trigger_event_ref: "order:" + _revOrderId,
                refunded_minor:    _refTotal,
                order_total_minor: _revSubtotal,
              });
            }).catch(function () { /* drop-silent — loyalty ledger holds its own audit trail */ });
          }
        }
    ...
    recordPartialRefund: async function (orderId, opts2) {
      ...
      // against the SUBTOTAL (the goods base the points were earned on), NOT
      // the grand total — refunding the full goods value claws all earned
      // points even when non-refundable shipping/tax is kept (the refund is
      // goods-first, capped at the subtotal). Clawing is not part of the
      ...
      if (_ptTotal > 0) {
        ...
        if (current.customer_id) {
          ...
          var _ptSubtotal = Number(current.subtotal_minor) || 0;
          if (_ptSubtotal > 0 && loyaltyEarnRules && typeof loyaltyEarnRules.reverseForEventProRata === "function") {
            Promise.resolve().then(function () {
              return loyaltyEarnRules.reverseForEventProRata({
                customer_id:       _ptCust,
                trigger_event_ref: "order:" + orderId,
                refunded_minor:    _ptRefunded,
                order_total_minor: _ptSubtotal,
              });
            }).catch(function () { /* drop-silent — loyalty ledger holds its own audit trail */ });
          }
        }
