...
function create(opts) {
  ...
  return {
    ...
    transition: async function (orderId, event, opts2) {
      ...
      // Denormalized history row. The fsm audit above is the authoritative
      // record, so a failure writing this row must NOT abort the inventory /
      // value settlement below: the status is already committed, and a retry
      // no-ops at the claim guard above (skipping settlement) — which would
      // strand the hold. Best-effort, drop-silent, with the fsm audit as trail.
      try {
        await query(
          "INSERT INTO order_transitions (id, order_id, from_state, to_state, on_event, reason, metadata_json, occurred_at) " +
          "VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)",
      ...
      } catch (_histErr) { /* drop-silent — fsm audit is authoritative; settlement must still run */ }
