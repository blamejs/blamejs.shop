...
function create(opts) {
  ...
  async function _collectSupportEvents(customerId, fromTs, toTs, boundLimit) {
    ...
    // outside still surfaces its opened event). Bounded read: a ticket
    // yields at most two events, so the newest `boundLimit` tickets by
    // opened_at cover any single page of `boundLimit` events. Unbounded
    // (summary path) keeps the full ASC scan for exact windowed counts.
    ...
    if (_isBound(boundLimit)) { supSql += "ORDER BY opened_at DESC LIMIT ?2"; supParams.push(boundLimit); }
