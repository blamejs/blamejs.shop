...
function create(opts) {
  ...
  return {
    ...
    // phantom inventory. creditOnce makes the credit exactly-once and
    // replay-safe. A credit never breaches the zero/holds floor, so this path
    // is positive-delta only.
    //
    // The credit and its idempotency record are ONE atomic write: the keyed
    // audit-row INSERT carries an AFTER INSERT trigger (migration 0233) that
    // upserts inventory_stock by NEW.delta in the SAME statement, so on D1 —
    // which has no interactive transactions — the credit and the key commit or
    // roll back together. There is no apply-vs-record gap to crash through. A
    // retry that re-presents the key inserts nothing (ON CONFLICT DO NOTHING),
    // so the trigger never fires and the shelf is never double-credited; an
    // INSERT that throws (e.g. the FK / CHECK on the trigger's upsert) leaves
    // neither the key nor the credit, so a retry re-attempts cleanly. The
    // trigger only fires for rows whose idempotency_key IS NOT NULL, so plain
    // adjustStock / transferStock audit rows (NULL key, already upserted in JS)
    // never get a second credit.
    creditOnce: async function (input) {
      ...
      var ts = _now();
      // The keyed insert IS the credit: ON CONFLICT DO NOTHING makes a replay a
      // no-op (no insert -> no trigger -> no second credit); a fresh key fires
      // the trigger that upserts the shelf atomically with the audit row.
      ...
      var row = await _getStockRow(input.sku, input.location_code);
      var qty = row ? row.quantity : 0;
      if (Number(changes || 0) === 0) {
        return {
          sku: input.sku, location_code: input.location_code,
          quantity: qty, delta: 0, applied: false,
        };
      }
      return {
        sku: input.sku, location_code: input.location_code,
        quantity: qty, delta: input.delta, applied: true,
      };
    },
