...
function create(opts) {
  ...
  return {
    ...
    //
    // Ordering note: the keyed audit row is written BEFORE the stock upsert, so
    // the only residual window is process death (isolate eviction) between the
    // two statements — which strands the credit at zero (under-credit) rather
    // than double-applying it. That is the safe direction and matches every
    // other claim-first transition in the codebase; the compensating delete
    // below covers the upsert-throws case so a retry re-attempts cleanly.
