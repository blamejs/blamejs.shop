...
function create(opts) {
  ...
  async function _audit(sku, locationCode, delta, reason) {
    await query(
      ...
      "VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
      [b.uuid.v7(), sku, locationCode, delta, reason || "", _now()],
    );
  }
  ...
  return {
    ...
    // phantom inventory. creditOnce makes the credit replay-safe by making the
    // audit row's idempotency_key the claim: the first call with a given key
    // inserts the row (winning the claim) and applies the credit; any later
    // call with the same key finds it present and is a no-op. A credit never
    // breaches the zero/holds floor, so this path is positive-delta only.
    //
    // Ordering note (deliberate, on D1's no-interactive-transaction model):
    // "apply the credit" and "record that the credit was applied" cannot be one
    // atomic write, so SOME single-statement gap is unavoidable and the choice
    // is only WHICH way it fails. The keyed audit row is written BEFORE the
    // stock upsert (claim-first). The compensating delete below covers the
    // common failure — the upsert throwing — so a retry re-attempts cleanly.
    // The only residual is the credit's host process dying (a container crash /
    // deploy roll — this runs in the persistent container, not a per-request
    // isolate) in the gap between the two statements: the key is durable but the
    // stock was not credited, so a retry no-ops and the shelf is UNDER-credited
    // until an operator recount, never double-credited. Under-credit is the safe
    // direction (a recoverable shortfall, not oversellable phantom stock) and
    // matches every other claim-first transition in the codebase. Writing the
    // key with/after the credit instead would invert this to a DOUBLE-credit on
    // the same gap, which is strictly worse for inventory integrity; a
    // claimed-vs-applied flag cannot close the gap either, since flipping it is
    // itself a separate non-atomic write with the same ambiguous middle state.
