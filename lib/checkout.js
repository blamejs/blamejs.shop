...
function create(deps) {
  ...
  async function _recordDiscountAllocation(quote, orderLines, orderId) {
    ...
    try {
      ...
      // line_id is the cart-line id carried through the quote; the per-
      // line subtotal is unit_amount_minor * qty — the same per-line
  ...
  return {
    ...
    confirm: async function (input) {
      ...
      if (amountDue === 0) {
        ...
        await _recordDiscountAllocation(quote, quote.lines, paidOrder.id);
      ...
      await _recordDiscountAllocation(quote, quote.lines, createdOrder.id);
