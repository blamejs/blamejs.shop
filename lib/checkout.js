...
function _email(s) {
  try { return _b().guardEmail.sanitize(s, { profile: "strict" }); }
...
function create(deps) {
  ...
  // discounts is optional — without it, every quote / confirm path
  // behaves as if no coupon was ever applied. With it, `quote()` and
  // `confirm()` accept an optional `discount_code` from the caller.
  var discounts = deps.discounts || null;
  ...
  async function _buildQuote(input) {
    ...
    // Resolve an optional coupon code. `resolve(...)` never throws on
    // refusal — it returns `{ discount_minor: 0, reason, discount? }`.
    // We pass the result through so the storefront can render either
    // "Coupon SUMMER25 applied" or "Coupon SUMMER25: expired".
    var resolved = null;
    if (input.discount_code != null && discounts) {
      resolved = await discounts.resolve({
        code:           input.discount_code,
        subtotal_minor: sub.amount_minor,
        currency:       c.currency,
      });
    }
    var discountMinor = resolved && !resolved.reason ? resolved.discount_minor : 0;
    ...
    return {
      ...
      totals:            totals,
      discount:          resolved,                 // null when no code submitted
    };
  }
  ...
  return {
    ...
    confirm: async function (input) {
      ...
      // Persist the redemption row last — only when an actually
      // applied (non-refused) coupon shaped the quote. `redeem(...)`
      // is atomic on the `uses` cap; if a concurrent confirmer just
      // exhausted the discount we surface the error to the caller so
      // the order can be rolled back / annotated. Refusal at redeem
      // time is rare in practice (the resolver already checked the
      // cap moments earlier) but the conditional-UPDATE guarantee
      // keeps it correct under concurrency.
      if (quote.discount && !quote.discount.reason && quote.discount.discount && quote.totals.discount_minor > 0) {
        await discounts.redeem(quote.discount.discount.id, createdOrder.id);
      }
