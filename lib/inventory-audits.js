...
function create(opts) {
  ...
  return {
    ...
    finalizeAudit: async function (input) {
        ...
        if (variance !== 0) {
          ...
          if (applyAdjustments) {
            ...
            if (line.location_code) {
              ...
              // finalizeAudit is re-runnable: a respect_holds floor
              // refusal (or a crash) can abort this loop before the
              // status flips below, leaving the audit open/in_progress
              // so the operator retries. adjustStock is NOT idempotent —
              // a second call re-debits/re-credits the shelf — so skip
              // any line whose adjustment row already landed under this
              // audit's reason. The audit's slug is unique and
              // (audit_id, sku, location_code) is UNIQUE, so
              // (sku, location_code, reason) names exactly one logical
              // adjustment; finding its row means the shelf already
              // moved and re-applying would double-count.
              var prior = await query(
                "SELECT 1 FROM inventory_adjustments WHERE sku = ?1 AND " +
                "location_code = ?2 AND reason = ?3 LIMIT 1",
                [line.sku, line.location_code, adjReason],
              );
              ...
              // Count every line whose adjustment is now in place
              // (this run or a prior partial run), so a completing retry
              // reports the audit's full applied total.
      ...
      var ts = _now();
      // Flip to finalized only while the audit is STILL open/in_progress.
      // The adjustments above are all applied (idempotently), so a
      // concurrent finalize that already flipped the row applied the same
      // numbers; gating the write on the non-terminal state collapses the
      // loser to a no-op instead of re-stamping finalized_at or
      // overwriting the winner's totals.
      await query(
