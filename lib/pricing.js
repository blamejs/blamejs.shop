 ...
 *
 * Composes b.i18n's number formatter so the storefront's locale +
 * currency rendering is consistent across all surfaces (cart
 * sidebar, checkout summary, email receipt). opts.locale overrides
 * the default locale from i18n; opts.minorUnits overrides ISO 4217
 * minor-unit count (useful for currencies like JPY which use 0
 * minor digits — i18n handles this automatically).
 */
function format(amountMinor, currency, opts) {
  ...
  // i18n's MessageFormat surface includes Intl.NumberFormat for
  // currency rendering. Build the formatter inline (cheap — the
  // call site is per-render, not per-tick).
  var nf = new Intl.NumberFormat(locale, {
    style:    "currency",
    currency: currency,
  });
  // Number.NumberFormat works in major units; convert from minor.
  // For most currencies (USD, EUR, GBP) that's /100; for JPY it's
  // /1 (no minor units). resolvedOptions().maximumFractionDigits
  // tells us the locale-correct divisor.
  var fractionDigits = nf.resolvedOptions().maximumFractionDigits;
  var divisor = Math.pow(10, fractionDigits);
  return nf.format(amountMinor / divisor);
}
