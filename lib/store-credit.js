...
function create(opts) {
  ...
  // refuses its INSERT (another write landed first). Each retry re-reads the
  // tip, so contention resolves in one extra round trip per racing writer.
  ...
  return {
    ...
    verifyChain: async function (customerId) {
      _uuid(customerId, "customer_id");
      var r = await query(
      ...
      return { ok: true, rows_verified: rows.length - legacyPrefix, legacy_prefix: legacyPrefix, last_hash: prevHash };
    },
