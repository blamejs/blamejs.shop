 ...
 *
 * `b.requestHelpers.trustedClientIp` is the primitive for this, and it carries
 * the peer gate this function lacks — a forwarded header honoured only when the
 * immediate socket peer is a configured proxy. It resolves `X-Forwarded-For`
 * only, though, and neither header read below is that one, so composing it here
 * would mean passing `clientIpResolver` and taking the whole trust decision
 * back. Requested upstream as blamejs/blamejs#560 (peer-gate a configurable
 * forwarded-IP header family); compose it once the header name is settable.
 */
