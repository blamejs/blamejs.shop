...
//
// b.render.stream with one thing added: the producer's first pull happens
// BEFORE the response is committed.
//
// The primitive opens the source before writing the status line, which covers
// a factory that throws and an object whose Symbol.asyncIterator throws. An
// async generator does neither — its body does not run until the first
// `next()` — so for the source type these routes actually use, "opened" still
// means no work has been attempted. A query that fails before yielding its
// first row therefore lands after the status line is out: the error handler
// can no longer produce a 4xx/5xx and the operator gets a committed attachment
// that resets mid-download instead of a page saying what went wrong.
//
// Pulling the first chunk here moves that failure back before commitment,
// where it reaches the error page normally. Everything after the first chunk
// is the primitive's: back-pressure, the closed peer, and a mid-stream failure
// broken rather than dressed up as a complete download.
//
// Reported upstream as blamejs/blamejs#567; delete this and call
// b.render.stream directly once a producer's first pull is inside the
// pre-commit window.
async function streamDownload(res, iterable, opts) {
  var makeIt = iterable[Symbol.asyncIterator] || iterable[Symbol.iterator];
  if (typeof makeIt !== "function") {
    throw new TypeError("streamDownload: iterable must be async- or sync-iterable");
  }
  var it = makeIt.call(iterable);
  // Not wrapped: a throw here is the point — it must reach the caller with the
  // status line still unsent.
  ...
  async function* rest() {
    ...
    try {
      ...
      // The primitive cancels by calling return() on what it was given, which
      // reaches this generator and not the one underneath it. Forward it, or a
      // client that disconnects mid-export leaves the cursor open.
      await _closeIterator(it);
    }
  }

  try {
    return await b.render.stream(res, rest(), opts);
  } finally {
    // A signal that is already aborted, or a response already finished, means
    // the primitive may never pull from `rest()` at all — so its `finally`
    // never runs. The first value has been taken from the producer by then,
    // and nothing else would ever close it.
    if (!started) await _closeIterator(it);
  }
}

async function _closeIterator(it) {
  if (typeof it.return !== "function") return;
  try { await it.return(); } catch (_e) { /* already closed */ }
}
