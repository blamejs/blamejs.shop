  ...
  return {
    ...
    redeem: async function (input) {
      ...
      try {
        ...
        // The balance was already debited by the atomic UPDATE above, but the
        // redemption row didn't land (a transient write fault). reverseRedemption
        // keys on that row, so without recovery this spend could never be
        // returned — the customer would lose the balance with no audit trail to
        // reverse. Re-credit the card (and un-terminate it if this debit had
        // zeroed it) so the redeem is all-or-nothing, then surface the error.
        await query(
          "UPDATE giftcards SET balance_minor = balance_minor + ?1, " +
