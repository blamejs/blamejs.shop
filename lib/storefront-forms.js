...
var THROTTLE_WINDOW_MS         = b.constants.TIME.minutes(1);
...
function _submitTo(input) {
  ...
  if (input.kind === "email") {
    ...
    return { kind: "email", value: b.guardEmail.sanitize(input.value, { profile: "strict" }) };
  }
  // webhook
...
function _validateValue(field, raw) {
  ...
  if (field.kind === "email") {
    ...
    var v = b.guardEmail.validate(raw, { profile: "strict" });
    if (!v.ok) {
      var ruleId = v.issues && v.issues[0] && v.issues[0].ruleId || "shape";
      return { ok: false, error: "email rejected (" + ruleId + ")" };
    }
    return { ok: true, value: b.guardEmail.sanitize(raw, { profile: "strict" }) };
  }
