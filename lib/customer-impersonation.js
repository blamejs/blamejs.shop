...
function create(opts) {
  ...
  return {
    ...
    notifyCustomer: async function (input) {
      ...
      // Claim the notification stamp BEFORE the enqueue side-effect.
      // The JS null-check above is advisory; without claiming the
      // stamp atomically, two concurrent callers both read a null
      // stamp, both enqueue, and the customer is notified twice. The
      // conditional UPDATE (`... WHERE customer_notified_at IS NULL`)
      // is the atomic claim — SQLite/D1 serializes writers, so exactly
      // one caller flips the stamp and earns the right to enqueue. The
      // loser re-reads the now-stamped row and returns it idempotently.
      var now = _now();
      ...
      if (Number(claim.rowCount || 0) !== 1) {
        // Lost the race — a concurrent notifyCustomer already claimed
        // (and enqueued). Re-read so we return that caller's stamp
        // rather than re-enqueuing.
        var current = await _getRow(id);
        return {
          notified:             true,
          customer_notified_at: current && current.customer_notified_at != null
                                  ? Number(current.customer_notified_at) : null,
        };
      }

      // Only the claim winner enqueues. If the enqueue throws, the
      // stamp must NOT persist — the row has to stay in the "needs
      // out-of-band notification" state so a retry re-attempts
      // delivery (silent drop would weaken the audit contract). Revert
      // the claim (self-targeting, so a concurrent end/expire that
      // touched other columns is untouched) before rethrowing.
      try {
        ...
        await query(
          "UPDATE impersonations SET customer_notified_at = NULL " +
          "WHERE id = ?1 AND customer_notified_at = ?2",
          [id, now],
        );
        throw e;
      }
