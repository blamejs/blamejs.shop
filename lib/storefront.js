...
function mount(router, deps) {
  ...
  if (deps.checkout && deps.order) {
    ...
    router.get("/orders/:order_id", async function (req, res) {
      ...
      // the customer's name, address, and line items by UUID alone. A
      // signed-in customer may only view their OWN orders here — viewing
      // someone else's by id 404s rather than leaking it. The guest
      // post-purchase confirmation path is preserved: a request with no
      // auth session (the just-placed-as-guest case) still renders, since
      // the checkout redirect lands the buyer here with no account.
      var orderAuth = _currentCustomerEnv(req);
      if (orderAuth && orderAuth.customer_id && o.customer_id !== orderAuth.customer_id) {
