...
function create(opts) {
  ...
  async function activeForPlacement(input) {
    ...
    for (var i = 0; i < rows.length; i += 1) {
      var banner = _hydrateRow(rows[i]);
      if (banner.audience === "all") return banner;
      if (banner.audience === "logged_in" && viewerKind === "logged_in") return banner;
      if (banner.audience === "guest"     && viewerKind === "guest")     return banner;
      if (banner.audience === "segment") {
        if (viewerKind !== "logged_in") continue;
        if (!customerSegments) {
          throw new TypeError("promoBanners.activeForPlacement: banner " + JSON.stringify(banner.slug) +
        ...
        var member = await customerSegments.isMember(customerId, banner.segment_slug);
        if (member) return banner;
        continue;
      }
    }
    return null;
